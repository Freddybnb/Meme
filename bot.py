...
def init_bot():
    """Initialisation en background après le démarrage de Flask"""
    time.sleep(2)
    ...
    try:
        ...
        if OKX_WALLET not in existing:
            ...
            print("[BOT] Wallet OKX ajouté dans EtherDrops")
        else:
            print("[BOT] Wallet OKX déjà suivi")
    except Exception as e:
        print(f"[BOT INIT ERROR] {e}")

    send_telegram(
        f"✅ <b>Bot démarré</b>\n\n"
        f"👁 Surveillance OKX wallet\n"
...
def webhook():
    ...
    try:
        ...
        for event in events:
            ...
            print(f"[EVENT] {from_addr[:16]}... -> {to_addr[:16]}... | {sol_amount:.3f} SOL | token={token_addr[:20]}")
